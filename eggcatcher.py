...
c.pack()
#declaration of different colour cycle of eggs
...
#creates the arc for catching the eggs
...
#movement of basket using keys to the left side
def move_left(event):
    (x1,y1,x2,y2) = c.coords(catcher)
    if x1 > 0:
        c.move(catcher,-20,0)
#movement of basket using keys to the right side
def move_right(event):
    (x1,y1,x2,y2) = c.coords(catcher)
    if x2 < canvas_width:
        c.move(catcher,20,0)
#binds the keys for the movement
